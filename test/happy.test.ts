...
test(`Should poll for a module and install it, then return the correct latest version`, async () => {
    await wrapPromise(async (reject) => {
        ...
        expect(
            // @ts-ignore - fs-extra types are incorrect for pify'ed exists
            await exists(join(result.modulePath, 'package.json'))
        ).toBeTruthy();
        ...
        for (const [ dependencyName, dependency ] of entries(
            ...
            expect(dependency.version).toEqual(
                MODULE_DEPENDENCIES[dependencyName]
            );
...
test(`Should poll for a module and install it with dependencies, then return the correct latest version`, async () => {
    await wrapPromise(async (reject) => {
        ...
        for (const [ dependencyName, dependency ] of entries(
            ...
            expect(
                // @ts-ignore - fs-extra types are incorrect for pify'ed exists
                await exists(join(dependency.path, 'package.json'))
            ).toBeTruthy();
        }
...
test(`Should poll for a module and install it, then import it`, async () => {
    await wrapPromise(async (reject) => {
        ...
        // @ts-ignore - import requires a path
